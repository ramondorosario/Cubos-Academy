...
const deletarAutor = (ctx) => {
	...
	if (resposta[0]) {
		...
		posts.forEach((item, i) => {
			// Verifica se o autor tem algum post. Se sim, será deletado
...
const atualizarAutor = (ctx) => {
	...
	if (resposta[0]) {
		if (autores[indiceAutor].deletado === false) {
			...
			if (
				...
				formatarErro(
					ctx,
					403,
					'Id ou Deletado não pode sofrer alteração'
				);
				return;
			}
		...
		if (!posts[indicePost].deletado) {
			...
			if (!autores[indiceAutor].deletado) {
				...
				formatarErro(
					ctx,
					404,
					'o post foi removido após o autor ser deletado'
				);
			}
		} else {
...
const atualizarPost = (ctx) => {
	...
	if (respostaPost[0]) {
		if (posts[indicePost].deletado === false) {
